...
def generate():
    ...
    seen_filter_bases = {}
    seen_dest_bases   = {}
    seen_dest_paths   = {}   # now keyed on (log_path, local_ident)
    ...
    for i, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        if local_ident:
            ...
            # Match-anything filter for rows with no ident, so we don't rely
            # on a degenerate regex against a single literal dot.
            filter_expr = 'match(".*" value("MSG") type("pcre"))'
        ...
        filter_blocks.append(
            ...
            f"    subst(\n"
            f'        "{local_ident if local_ident else "UNKNOWN"} ",\n'
            f'        "{local_ident if local_ident else "UNKNOWN"}/${{SOURCEIP}} {facility} ",\n'
            f'        value("MSG")\n'
            f'        type("pcre")\n'
            f"    );\n"
            f"}};\n"
        ...
        if log_path:
            dedup_key = (log_path, local_ident)
            if dedup_key in seen_dest_paths:
                dest_name = seen_dest_paths[dedup_key]
            else:
                dest_base = "d_" + (sanitize_name(local_ident) if local_ident else f"row{i}")
                dest_name = make_unique(dest_base, seen_dest_bases)
                seen_dest_paths[dedup_key] = dest_name
                dest_blocks.append(
